...
out = subprocess.Popen(['eos', const.eosPath, 'ls', skimFolder_4btag_and_3btag[1]], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
for fileName in stdout.split():
    if ".root" in fileName:
		skimFileListOfList[listNumber].append(const.eosPath + "/" + skimFolder_4btag_and_3btag[1] + "/" + fileName)
