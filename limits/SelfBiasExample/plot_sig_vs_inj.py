...
def stupitPythonMean(valueList):
    sum = 0
...
def stupitPythonStdDev(valueList):
    sum = 0
    sumSquare = 0
    for value in valueList:
        sum += value
        sumSquare += (value*value)
    return sqrt(sumSquare - sum*sum)
...
def PrepareFitGraph(injlist,fIn_proto):
    ...
    for inj in injlist:
        ...
        meanerror = statistics.stdDev(vals)/math.sqrt(i)
        stvd      = statistics.stdDev(vals)  
