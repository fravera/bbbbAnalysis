...
// cd privateTools && g++  -std=c++17 -g -I `root-config --incdir` -o PlotCutFlowVsMy PlotCutFlowVsMy.cc `root-config --libs` -O3; cd ..
...
class CutFlowPlotter
{
  public:
    ...
    void addPoint(int mY)
    {
        fMyList.emplace_back(mY);
        std::sort(fMyList.begin(), fMyList.end());
    }
    ...
    void createCutContainer(std::vector<CutFlowSelection::CutInfo> theCutInfoList)
    {
        ...
        {
            fEfficiencyPairList.emplace_back(std::make_pair(std::get<0>(theCutInfo), std::make_tuple(std::vector<float>(fMyList.size(), 0.), std::get<3>(theCutInfo))));
        }
    }

    void fill(int mY, const std::shared_ptr<TH1F> theCutFlowPlot)
    {
        size_t index = find(fMyList.begin(), fMyList.end(), mY) - fMyList.begin();
    ...
    std::shared_ptr<TCanvas> producePlots()
    {
        ...
        for(auto& cut : fEfficiencyPairList)
        {
            TGraph *theGraph = new TGraph(fMyList.size(), fMyList.data(), std::get<0>(cut.second).data());
            ...
            {
                isFirstPlot = false;
                theGraph->SetTitle(("Cut flow efficiency - m_{X} = " + std::to_string(fMX) + " GeV - Run " + std::to_string(fYear)).c_str());
                theGraph->GetYaxis()->SetRangeUser(0., 1.1);
                theGraph->GetYaxis()->SetTitle("efficiency");
                theGraph->GetXaxis()->SetTitle("m_{Y} [GeV]");
...
int main(int argc, char** argv)
{
    ...
    for(const auto& signal : signalList)
    {
        ...
        if (theCutFlowPlotList.find(mX) == theCutFlowPlotList.end()) theCutFlowPlotList[mX] = CutFlowPlotter(year, mX);
        theCutFlowPlotList[mX].addPoint(mY);
    }
    ...
    auto produceCutFlowPlot = [&outputFile, &folderName, &theCutFlowPlotList, year](std::string& sampleName)
    {
        ...
        theCutFlowPlotList[mX].fill(mY, cutFlowPlot.first);
    ...
    for(auto& theCutFlowPlot: theCutFlowPlotList)
    {
        ...
        std::string canvasName = "CutFlow_MX_" + std::to_string(theCutFlowPlot.first) + "_" + std::to_string(year);
